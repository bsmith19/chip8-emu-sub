...
class InstAssign_8XY0Tests : public ::testing::Test
{
public:
    void SetUp();
    void TearDown();
...
void InstAssign_8XY0Tests::SetUp()
{
    // create memory map
...
void InstAssign_8XY0Tests::TearDown()
{
}

TEST_F(InstAssign_8XY0Tests, handle_test)
{
    ...
    InstAssign_8XY0* in = new InstAssign_8XY0(0x8120);
    EXPECT_TRUE(in->Handle(mSystemData));
    
    EXPECT_EQ(mSystemData->systemCpuRegisters[2], 7);
    EXPECT_EQ(mSystemData->systemCpuRegisters[1], 7);
